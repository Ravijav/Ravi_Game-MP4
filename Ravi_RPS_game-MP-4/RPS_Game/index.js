...
addBoxButton1.addEventListener('click',() =>{
        ...
        if (parseInt(users.textContent) === 15 || parseInt(comps.textContent) === 15) {
            hiddenBox1.classList.remove('hidden'); // Unhide hiddenBox1 when users' value becomes 15
            users.textContent = 0;
            comps.textContent = 0;
            hiddenBox1.classList.add('hidden'); // Unhide hiddenBox1 when users' value becomes 15
